...
# ilocメソッド：インデックス番号・カラム番号を指定して値を抽出
print("df:=", df)
print("df.iloc[1,1]:=", df.iloc[1, 1])  # インデックス、カラムを位置で指定
print("df.iloc[1:,1]:=", df.iloc[1:, 1])  # インデックスを範囲、カラムを位置で指定
print("df.iloc[1:,:2]:=", df.iloc[1:, :2])  # インデックス、カラムを範囲で指定
